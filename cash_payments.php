...
try {
    $stmt = $pdo->prepare("SELECT DISTINCT heading FROM cash_payments WHERE heading IS NOT NULL ORDER BY heading");
    $stmt->execute();
    $headings = $stmt->fetchAll(PDO::FETCH_COLUMN);

    $stmt = $pdo->prepare("SELECT DISTINCT sub_heading FROM cash_payments WHERE sub_heading IS NOT NULL ORDER BY sub_heading");
    $stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_payment'])) {
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO cash_payments (date, description, heading, sub_heading, amount, user_id) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$date, $description, $heading, $sub_heading, $amount, $user_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_payment'])) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT amount, date FROM cash_payments WHERE id = ?");
        ...
        $stmt = $pdo->prepare("UPDATE cash_payments SET date = ?, description = ?, heading = ?, sub_heading = ?, amount = ? WHERE id = ?");
        $stmt->execute([$date, $description, $heading, $sub_heading, $new_amount, $payment_id]);

        // Reverse original amount from cash on hand
        $stmt = $pdo->prepare("INSERT INTO cash_on_hand (date, amount) VALUES (?, ?) ON DUPLICATE KEY UPDATE amount = amount + ?");
        $stmt->execute([$original_date, $original_amount, $original_amount]);

        // Deduct new amount from cash on hand
        $stmt = $pdo->prepare("INSERT INTO cash_on_hand (date, amount) VALUES (?, ?) ON DUPLICATE KEY UPDATE amount = amount - ?");
        $stmt->execute([$date, -$new_amount, $new_amount]);
...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT cp.id, cp.date, cp.description, cp.heading, cp.sub_heading, cp.amount, u.username 
...
<head>
    ...
    <title>SL Diagnostics - Cash Payments</title>
    ...
    <style>
        ...
        #new_heading_container, #new_sub_heading_container {
            display: none;
        }
        @media print {
            body * {
...
<body>
    ...
    <div class="content">
        <h1 class="display-5 mb-4">Cash Payments</h1>
        ...
        <div class="card payment-table">
            <div class="card-header">
                ...
                <form class="d-inline-flex align-items-center">
                    <label for="date-filter" class="form-label me-2 mb-0">Date:</label>
            ...
            <div class="card-body">
                ...
                <?php else: ?>
                            ...
                            <tbody>
                                <?php foreach ($payments as $payment): ?>
                                    <tr>
                                        ...
                                        <td><? Cang echo htmlspecialchars($payment['date']); ?></td>
                    ...
                    <button class="btn btn-primary mt-3 print-btn" onclick="window.print()">Print Payment Records</button>
                <?php endif; ?>
            </div>
        </div>
    </div>
