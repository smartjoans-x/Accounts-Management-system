...
if (file_exists($env_file)) {
    ...
    if ($env === false) {
        ...
        error_log("Successfully loaded .env file at $env_file");
    }
} else {
...
$diagnostic_db_host = $env['DIAGNOSTIC_DB_HOST'] ?? '192.168.0.161:3306';
$diagnostic_db_name = $env['DIAGNOSTIC_DB_NAME'] ?? 'sldiagnostic';
$diagnostic_db_user = $env['DIAGNOSTIC_DB_USER'] ?? 'root';
$diagnostic_db_pass = $env['DIAGNOSTIC_DB_PASS'] ?? '1234567';
...
error_log("sldiagnostic Config: Host=$diagnostic_db_host, User=$diagnostic_db_user, DB=$diagnostic_db_name, Pass=" . ($diagnostic_db_pass ? '[set]' : '[unset]'));
...
try {
    ...
    echo "Database connection error: Unable to connect to accounts database: " . htmlspecialchars($e->getMessage()) . "<br>";
    exit;
} catch (Throwable $e) {
    error_log("Unexpected error in sld_accounts connection: " . $e->getMessage());
    echo "System error: Unable to connect to accounts database: " . htmlspecialchars($e->getMessage()) . "<br>";
    exit;
}

// PDO connection for sldiagnostic
$pdo_diagnostic = null;
try {
    error_log("Attempting connection to sldiagnostic...");
    $pdo_diagnostic = new PDO(
        "mysql:host=$diagnostic_db_host;dbname=$diagnostic_db_name;charset=utf8mb4",
        $diagnostic_db_user,
        $diagnostic_db_pass,
        [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC]
    );
    $pdo_diagnostic->query("SELECT 1");
    error_log("Successfully connected to sldiagnostic database.");
} catch (PDOException $e) {
    error_log("sldiagnostic database connection failed: " . $e->getMessage());
    echo "Database connection error: Unable to connect to diagnostic database: " . htmlspecialchars($e->getMessage()) . "<br>";
    exit;
} catch (Throwable $e) {
    error_log("Unexpected error in sldiagnostic connection: " . $e->getMessage());
    echo "System error: Unable to connect to diagnostic database: " . htmlspecialchars($e->getMessage()) . "<br>";
    exit;
}

// Verify PDO objects
if (!$pdo_accounts instanceof PDO || !$pdo_diagnostic instanceof PDO) {
    error_log("PDO objects not properly initialized.");
    die("System error: Database connections not initialized.");
}
...
try {
    ...
    error_log("Error fetching cumulative cash on hand: " . $e->getMessage());
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_fuel'])) {
    try {
        ...
        $stmt = $pdo_diagnostic->prepare("UPDATE vehicles SET total_km = ? WHERE vehicle_name = ?");
        $stmt->execute([$km_reading, $vehicle_name]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_fuel'])) { // Line 179
    try {
        ...
        $stmt = $pdo_accounts->prepare("SELECT vehicle_name, km_reading, fuel_amount, payment_method, bank_account_id FROM vehicle_fuel_records WHERE id = ?");
        $stmt->execute([$fuel_id]);
        $original_fuel = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$original_fuel) { // Line 191 (fixed)
            throw new Exception("Fuel record not found.");
        }
        $original_vehicle_name = $original_fuel['vehicle_name'];
        $original_km_reading = $original_fuel['km_reading'];
        ...
        $stmt = $pdo_diagnostic->prepare("UPDATE vehicles SET total_km = ? WHERE vehicle_name = ?");
        $stmt->execute([$new_km_reading, $vehicle_name]);
...
} // End edit fuel record

// Fetch vehicles
$vehicles = [];
try {
    $stmt = $pdo_diagnostic->query("SELECT vehicle_name, total_km FROM vehicles");
    $vehicles = $stmt->fetchAll(PDO::FETCH_ASSOC);
    error_log("Fetched " . count($vehicles) . " vehicles.");
} catch (PDOException $e) {
    error_log("Error fetching vehicles: " . $e->getMessage());
    echo "Error loading vehicles: " . htmlspecialchars($e->getMessage()) . "<br>";
}
...
<body>
    ...
    <div class="content">
                ...
                <form method="POST">
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-control" id="vehicle_name" name="vehicle_name" required onchange="updateKmReading()">
                                <option value="">Select Vehicle</option>
                                <?php foreach ($vehicles as $vehicle): ?>
                                    <option value="<?php echo htmlspecialchars($vehicle['vehicle_name']); ?>" data-km="<?php echo htmlspecialchars($vehicle['total_km']); ?>">
        ...
        <div class="card fuel-table">
            <div class="card-header">Fuel Records</div>
            <div class="card-body">
                <?php if (empty($fuel_records)): ?>
                    <p class="text-muted">No fuel records found.</p>
                <?php else: ?>
                            ...
                            <tbody>
                                <?php foreach ($fuel_records as $record): ?>
                                    <tr>
                                        ...
                                        <td><?php echo $record['bank_account_id'] ? htmlspecialchars($record['account_number']) : '-'; ?></td>
                                                ...
                                                <div class="modal-body">
                                                    <form method="POST">
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <select class="form-control" id="edit_vehicle_name_<?php echo $record['id']; ?>" name="vehicle_name" required onchange="updateEditKmReading(<?php echo $record['id']; ?>)">
                                                                <option value="">Select Vehicle</option>
                                                                <?php foreach ($vehicles as $vehicle): ?>
                                                                    <option value="<?php echo htmlspecialchars($vehicle['vehicle_name']); ?>" data-km="<?php echo htmlspecialchars($vehicle['total_km']); ?>" <?php echo $vehicle['vehicle_name'] == $record['vehicle_name'] ? 'selected' : ''; ?>>
                                                                        <?php echo htmlspecialchars($vehicle['vehicle_name']); ?>
                                                                    </option>
                                                                <?php endforeach; ?>
                                                            </select>
    ...
    <script>
        function updateKmReading() {
            const select = document.getElementById('vehicle_name');
            const kmInput = document.getElementById('km_reading');
            const selectedOption = select.options[select.selectedIndex];
            kmInput.value = selectedOption ? selectedOption.getAttribute('data-km') || '' : '';
        }

        function updateEditKmReading(recordId) {
            const select = document.getElementById('edit_vehicle_name_' + recordId);
            const kmInput = document.getElementById('edit_km_reading_' + recordId);
            const selectedOption = select.options[select.selectedIndex];
            kmInput.value = selectedOption ? selectedOption.getAttribute('data-km') || '' : '';
        }
        ...
        function toggleBankAccount() {
            ...
            console.log('Toggling bank account, payment method:', paymentMethod); // Debug
        ...
        function toggleEditBankAccount(recordId) {
            ...
            console.log('Toggling edit bank account for record', recordId, 'payment method:', paymentMethod); // Debug
            container.style.display = paymentMethod === 'Bank' ? 'block' : 'none';
            select.required = paymentMethod === 'Bank';
        }
